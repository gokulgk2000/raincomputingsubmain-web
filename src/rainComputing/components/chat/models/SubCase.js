...
const SubCase = ({ caseId, open, setModalOpen, ongetAllCases, newCaseId }) => {
    console.log('caseId', caseId);
    const history = useHistory();
    const { currentUser } = useUser();
    const [contacts, setContacts] = useState([]);
    const [caseName, setCaseName] = useState('');
    const [caseIds, setCaseId] = useState(newCaseId);
    ...
    const handleCreateSubCase = async () => {
        ...
        const payload = {
            ...
            aflag: true,
            maincaseId: caseId?.caseId,
            isSubcase: true,
        };

        const res = await createNewCase(payload);
        if (res.success) {
            toastr.success(
            ...
            history.push(`/chat-rc?g_id=${res?.group}&c_id=${res?.case}`);
            setModalOpen(false);
        } else {
